...
def snowconnection(connection_config):
    ...
    session_details = session.create_dataframe([[session.session_id,str(session.get_current_user()),str(session.get_current_warehouse()).replace('"',''),str(session.get_current_role()).replace('"','')]], schema=["session_id","user_name","warehouse","role"])
